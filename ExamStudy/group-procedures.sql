...
DROP PROCEDURE IF EXISTS GetUsersGroups;
DROP PROCEDURE IF EXISTS GetAdminMembers;
...
DELIMITER $$
CREATE PROCEDURE `GetUsersGroups`(
	IN p_userId INT
)
BEGIN
	SELECT * FROM GroupDetailed G
...
DELIMITER $$
CREATE PROCEDURE `GetAdminMembers`(
	IN p_GroupId INT
)
BEGIN
	SELECT UserId FROM GroupMembers
	WHERE GroupId = p_GroupId AND MemberType IN ('OWNER', 'ADMIN');
END $$
DELIMITER ;
